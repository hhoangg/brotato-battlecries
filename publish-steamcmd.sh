...
# Publish the Battle Cries voice mod to the Steam Workshop via SteamCMD — WITHOUT
...
# workshop_item.vdf's "publishedfileid" so every later run UPDATES that same item instead
...
# 1) Build the mod zip (stages the voice clips in).
